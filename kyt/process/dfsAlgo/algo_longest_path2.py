...
# Used
def _findLongestPathRec2( aUData, aGraph, aNode, aWishList, aWishLeaves, aAllPaths, aWithCycles, aVisited=None, aCurrPath=tuple() ):
    retVal= None
...
#ok version        if aNode.isLeaf():
        if aNode.isLeaf() or ( aWishLeaves and aNode._num in aWishLeaves):
            ...
            for iNodeNum in aNode._edges:
                if int(iNodeNum) in aVisited:
                    ...
                    vRes = _findLongestPathRec2( aUData, aGraph, aGraph.node(iNodeNum), aWishList, aWishLeaves, aAllPaths, aWithCycles, aVisited, vCurrPath )
...
def findLongestPath2( aGraph, aRootNode, aObjectsOfInterest, aLeavesOfInterest, aCycles ):
    retAllPaths = { aRootNode._num : [] }
    aUData = collections.namedtuple("TUdata",["start"])(time.perf_counter())
    try:
        retRes = _findLongestPathRec2( aUData, aGraph, aRootNode, aObjectsOfInterest, aLeavesOfInterest, retAllPaths, aCycles )
...
def findLongestPathNoCycles2( aGraph, aRootNode, aObjectsOfInterest, aLeavesOfInterest ):
    return findLongestPath2( aGraph, aRootNode, aObjectsOfInterest, aLeavesOfInterest, False )

def findLongestPathWithCycles2( aGraph, aRootNode, aObjectsOfInterest, aLeavesOfInterest ):
    return findLongestPath2( aGraph, aRootNode, aObjectsOfInterest, aLeavesOfInterest, True)
